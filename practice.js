...
const fullName = (firstName, lastName, welcomeMessageCb) => welcomeMessageCb(firstName, lastName);
...
fullName("Eva", "Przybyla", welcomeMessage)
...
const canDrink = ( (age, drinkSoda, drinkAlcohol) => {
  if (age < 21) {
   return drinkSoda();
  } else if (age >= 21) {
   return drinkAlcohol();
...
const math = (num1, num2, operator) => operator(num1, num2);
...
//Code Here
let postTaxPrices = prices.map(price => price + price * .07); // = prices.map(/* Provide Your Callback Here );
...
}, initValue); //  = populations.reduce(/* Provide Your Callback Here */)

////////// PROBLEM 12 //////////

// BONUS POINTS Add all CP per pokemon species, filter out all that start with w, and lowercase all names
...
// filter out all names with w
// use the bang operator to return non w names 
// lowercase All names
let filteredMonsters = monstersInYourPocket.filter((monster) => !monster.monster.startsWith("W"));

let mappedMonsters = filteredMonsters.map((monster) => {
  return {
    monster: monster.monster.toLowerCase(),
    CP: monster.CP
  }
})
...
//   return {
//     ...dict,
//     [currentMonster.monster]: (dict[currentMonster.monster] || 0) + currentMonster.CP
//   }
// }, {});

// console.log('reducedMonsters', reducedMonsters)
